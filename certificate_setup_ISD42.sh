...
scion-pki certificate create --profile=cp-root <(echo '{"isd_as": "42-ffaa:1:3", "common_name": "42-ffaa:1:3 cp root cert"}') cp-root.pem cp-root.key
popd
...
core_ases = ["ffaa:1:1", "ffaa:1:3"]
authoritative_ases = ["ffaa:1:1", "ffaa:1:3"]
cert_files = ["AS1/sensitive-voting.pem", "AS1/regular-voting.pem", "AS1/cp-root.pem", "AS3/cp-root.pem", "AS3/sensitive-voting.pem", "AS3/regular-voting.pem"]
...
pushd AS3
scion-pki certificate create --profile=cp-ca <(echo '{"isd_as": "42-ffaa:1:3", "common_name": "42-ffaa:1:3 CA cert"}') cp-ca.pem cp-ca.key --ca cp-root.pem --ca-key cp-root.key
popd
...
scion-pki certificate create --profile=cp-as <(echo '{"isd_as": "42-ffaa:1:2", "common_name": "42-ffaa:1:2 AS cert"}') AS2/cp-as.pem AS2/cp-as.key --ca AS1/cp-ca.pem --ca-key AS1/cp-ca.key --bundle
scion-pki certificate create --profile=cp-as <(echo '{"isd_as": "42-ffaa:1:3", "common_name": "42-ffaa:1:3 AS cert"}') AS3/cp-as.pem AS3/cp-as.key --ca AS3/cp-ca.pem --ca-key AS3/cp-ca.key --bundle
